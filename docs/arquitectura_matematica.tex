...
\begin{equation}
  h = \tanh\!\bigl( W_1\, [\,X,\; t\,\mathbf{1}\,]^\top + b_1\bigr)
  \;\in\; \R^{N \times M},
  \qquad
...
Stochastic Gradient Langevin Dynamics}, Kim et al.~\cite{li2016psgld}):
...
Kim et al.\ en su implementación original. Con esta cota, el ruido
...
destruye la distribución estacionaria. Con el acoplamiento
de~\eqref{eq:psgld}--\eqref{eq:precond}, la SDE efectiva es la Langevin
precondicionada
\begin{equation}
  d\theta = -\nabla J\,dt + \sqrt{2\varepsilon M}\,dW,
\end{equation}
cuya medida estacionaria es $\tilde{\nu} \propto \exp(-J(\theta)/\varepsilon)$
bajo el precondicionador $M$ (resultado estándar de difusiones precondicionadas).

\paragraph{Resultado y limitación respecto al paper.}
El ruido de Langevin hace que los parámetros exploren $\tilde{\nu}$ en lugar
de colapsar al MAP, aproximando así el término de entropía $-H(\nu_t)$ de la
KL. El término energético $\mathcal{E}$ en~\eqref{eq:energia} actúa como el
prior $\nu^\infty$. Con $\varepsilon = 0$ el ruido es exactamente cero (Adam
estándar).

Sin embargo, $\tilde{\nu}$ \textbf{no coincide} con la medida de control
óptima $\nu^*_t$ del paper. Según la condición de primer orden
(principio de Pontryagin), $\nu^*_t$ tiene la forma
\begin{equation}
  \nu^*_t(a) \propto \exp\!\Bigl(-\ell(a)
  - \tfrac{1}{\varepsilon}\int b(x,a)\cdot\nabla_x u_t(x,y)\,d\gamma_t(x,y)\Bigr),
\end{equation}
donde $u_t$ es el estado adjunto que satisface la ecuación backward acoplada
con la ecuación forward de $\gamma_t$. El potencial de $\tilde{\nu}$ es
$J(\theta)/\varepsilon$ (la pérdida total evaluada en los parámetros), mientras
que el de $\nu^*_t$ involucra la interacción entre el campo $b(x,a)$ y el
gradiente del adjunto $\nabla_x u_t$: una cantidad que requiere resolver el
sistema forward-backward completo y que la implementación actual no computa.

\textit{Uso de pSGLD.} A pesar de esta brecha,
pSGLD es la elección más coherente disponible.
La alternativa directa ---Adam sin ruido con $\varepsilon > 0$--- reduce
$\varepsilon$ a un simple \emph{weight decay} polinomial, perdiendo por
completo el rol de temperatura entrópica que tiene en el paper.
pSGLD, en cambio, hace que $\varepsilon$ controle simultáneamente la
regularización (vía $\mathcal{E}$) y la exploración del espacio de parámetros
(vía el ruido de Langevin), que es precisamente el doble rol que le asigna
el paper. La distribución $\tilde{\nu}$ es por tanto la mejor aproximación
a $\nu^*_t$ accesible sin resolver el sistema forward-backward.
...
$\varepsilon \in \{0,\; 10^{-3},\; 10^{-2},\; 0.1,\; 0.5\}$
con \textbf{pSGLD}
(\texttt{use\_sgld=True}, Kim et al.~\cite{li2016psgld}), manteniendo la
...
$\tilde{\nu} \propto \exp(-J(\theta)/\varepsilon)$, aproximación de la medida
óptima $\nu^*_t$ del paper (véase Sección~\ref{sec:sgld}).
...
\begin{itemize}
  \item \textbf{B1 — Curvas de convergencia:} $J$ total, accuracy y
        $\mathcal{E}$ vs.\ época. Los ejes están recortados ($J \leq 0.5$,
        $\mathcal{E} \leq 0.15$) para que $\varepsilon=0.5$ no domine la escala.
  \item \textbf{B2 — Fronteras de decisión y campo de velocidad:} región de
        decisión $\{x : \hat{p}(x) \geq 0.5\}$ y quiver plot de $F(x, t=0.5)$
        para cada $\varepsilon$. Fronteras más suaves y campo más regular al
        crecer $\varepsilon$.
\end{itemize}

\textbf{Efecto de $\varepsilon$ grande.}
Con $\varepsilon$ pequeño ($\leq 0.01$), el término entrópico $\varepsilon\cdot\mathcal{E}$
es un ajuste fino: el modelo minimiza principalmente la BCE.
Con $\varepsilon = 0.5$, la regularización pesa 50 veces más que con $\varepsilon=0.01$
y se convierte en un objetivo \emph{competitivo} con la BCE: el campo $F$ necesita
parámetros grandes para separar las lunas, pero $\varepsilon$ los penaliza
fuertemente. El resultado es un compromiso — parámetros moderados que no
separan perfectamente las clases — de ahí que la accuracy no llegue a $1.0$.
Con $\varepsilon = 0.1$ el efecto es intermedio: la pérdida $J$ no converge a
cero porque el término entrópico mantiene un nivel asintótico elevado.
En ambos casos el ruido pSGLD, que escala como $\sqrt{\varepsilon}$, amplifica
adicionalmente las oscilaciones de las curvas.
...
\begin{equation}
  ...
  \right\}.
\end{equation}
Se usa el percentil 10 en lugar de la media o el mínimo porque el ratio
$G_s/(2\delta_s)$ puede ser ruidoso en las épocas iniciales (cuando $\delta_s$
es grande y el gradiente aún no ha encontrado la dirección de descenso) y en
las épocas finales (cuando $\delta_s \approx 0$ y el cociente diverge
numéricamente). El percentil 10 descarta esos valores extremos y proporciona
una cota inferior robusta: garantiza que la condición PL se cumple con
constante $\hat{\mu}$ en al menos el $90\%$ de las épocas.

\textbf{Paneles generados.}
\begin{itemize}
  \item \textbf{C1 — Log-log:} $G_s$ vs.\ $\delta_s$. Los puntos deben quedar
        por encima de la recta $G = 2\hat{\mu}\,\delta$. La pendiente $\approx 1$
        en escala log-log es la firma visual de la condición PL: $\|\nabla J\|^2$
        crece proporcionalmente a $(J - J^*)$.
  \item \textbf{C2 — Semilog:} $\delta_s$ vs.\ época. Caída exponencial
        confirma la convergencia predicha por~\eqref{eq:pl}. Con SGD + lr
        constante el aplanamiento final refleja convergencia genuina al mínimo,
        sin el artefacto del cosine annealing que reduciría $\delta_s$
        artificialmente al llevar $\eta_s \to 0$.
  \item \textbf{C3 — Barras:} $\hat{\mu}_{\text{PL}}$ para cada $\varepsilon$.
        Confirma $\hat{\mu} > 0$ para todo $\varepsilon > 0$. No se garantiza
        que $\hat{\mu}$ crezca con $\varepsilon$: valores grandes de $\varepsilon$
        elevan $J^*$, lo que puede reducir el ratio $G_s / (2\delta_s)$.
  \item \textbf{C4 — Ratio PL:} $G_s / (2\delta_s)$ vs.\ época (escala log),
        que debe mantenerse $\geq \hat{\mu}$ en todo momento. Que se cumpla
        también en las épocas tardías confirma que la PL es una propiedad
        global del paisaje de $J$, no solo válida al inicio del entrenamiento.
\end{itemize}
...
\textbf{Observación sobre D2.} Empíricamente, la banda de D2 es casi
invisible: con inicialización fija y datasets distintos, las curvas de
pérdida son prácticamente idénticas entre sí. Esto se debe a que todos los
datasets generados por \texttt{make\_moons} con $N=400$ y \texttt{noise}$=0.12$
son estadísticamente muy similares — distintas realizaciones de la misma
distribución con suficientes puntos. La trayectoria de optimización queda
determinada principalmente por $\theta_0$, y la variabilidad específica de
cada muestra es despreciable. Es la evidencia más directa del Meta-Teorema~1:
lo que importa es la distribución $\gamma_0$, no la realización concreta.

\subsection{Experimento E: Robustez de $\nu^*$ (make\_moons)}

\textbf{Objetivo.} Verificar que la distribución de parámetros aprendida por
pSGLD ---aproximación empírica de la medida óptima $\nu^*_t$ del paper--- es
robusta a distintas condiciones de entrenamiento: distintas inicializaciones
y distintos datasets deben producir distribuciones similares.

\textbf{Protocolo.} Dos sub-experimentos con $n_\text{seeds} = 10$,
$\varepsilon = 0.01$, 500 épocas y \textbf{pSGLD} (\texttt{use\_sgld=True},
para explorar $\tilde{\nu} \propto \exp(-J(\theta)/\varepsilon)$ en lugar del MAP):
\begin{itemize}
...
\textbf{Relación con el Experimento D y finalidad.}
El Experimento~D mostró que $J^*$ es robusto entre seeds: distintas condiciones
de entrenamiento convergen al mismo valor de pérdida. Pero esto no implica
que la \emph{distribución} de parámetros $\nu^*$ sea la misma — dos modelos
con igual $J^*$ podrían haber llegado ahí con estructuras internas
completamente distintas.

El Experimento~E va un paso más allá: verifica que la estructura interna
de $\nu^*$ es también robusta. Si el Meta-Teorema~1 se cumple (minimizador
único estable), las curvas de importancia deben ser cualitativamente similares
entre seeds: mismo ``codo'' pronunciado en los primeros ranks (pocas neuronas
concentran la mayor parte del peso efectivo) y misma caída. Una banda estrecha
en ambos paneles confirma que $\nu^*$ no es solo un mínimo con el mismo coste,
sino genuinamente la \emph{misma} distribución de parámetros.
...
\textbf{Métrica de convergencia.} Con Adam o SGD el optimizador converge a un
punto y $J^*$ es el valor mínimo alcanzado — un número bien definido.
Con pSGLD, en cambio, los parámetros \emph{no} convergen a un punto: el ruido
de Langevin los mantiene explorando permanentemente alrededor del mínimo.
Reportar el mínimo $J^*$ sería engañoso, porque en cualquier época el valor
de $J$ puede estar por encima por culpa del ruido, no por falta de convergencia.
Por eso se usa $\bar{J}_\text{final}$: la media de las últimas 50 épocas,
que estima el nivel estacionario de la cadena de Markov una vez que ha
``llegado'' a la región del mínimo y oscila alrededor de él.

\textbf{Figura y conclusión.} Se muestra un único panel con las curvas de
convergencia $J \pm 1\sigma$ para F1 (rojo) y F2 (azul) superpuestas.
Empíricamente, la curva de F1 queda casi siempre por debajo de la de F2:
fijar la inicialización y variar los datos introduce menos variabilidad
que fijar los datos y variar la inicialización. Esto indica que, en
\texttt{make\_circles}, la fuente de variabilidad dominante es la
inicialización de parámetros $\theta_0$, no el dataset concreto — resultado
coherente con la observación análoga en D2 para \texttt{make\_moons}.
...
\textbf{Marco teórico.} El paper \cite{daudin2025} trabaja en el régimen de
campo medio ($M \to \infty$ neuronas) con distribución inicial \emph{continua}
$\gamma_0$. El caso discreto con $N$ datos finitos queda fuera del alcance
del paper: en la Sección~1.6 (punto 3) los autores lo señalan explícitamente
como extensión futura. La convergencia $J^*_N \to J^*_\infty$ cuando
$N \to \infty$ es por tanto una \textbf{conjetura} motivada por la intuición
estadística, no un resultado demostrado. Una tasa natural esperada por
analogía con la ley de los grandes números sería
$\text{gap}(N) := J^*_\infty - J^*_N \sim C \cdot N^{-\alpha}$ con
$\alpha \approx 0.5$. El experimento busca evidencia empírica de esta
convergencia y estima la tasa $\alpha$.

\textbf{Protocolo.} Se consideran $N \in \{25, 50, 100, 200, 400, 800\}$
partículas: una progresión geométrica de razón~2 que cubre dos órdenes de
magnitud, desde un régimen de datos escasos ($N=25$, apenas suficientes
para definir las dos lunas) hasta un régimen rico ($N=800$, donde la
distribución empírica aproxima bien a $\gamma_0$). El valor $N=400$ es el
estándar del resto de experimentos, lo que permite comparar directamente.
Se eligen $\varepsilon \in \{0, 0.01\}$ para contrastar el caso sin
regularización entrópica con el caso de referencia del paper.
Para cada combinación $(N, \varepsilon)$ se realizan 20 entrenamientos
independientes (\texttt{n\_seeds}$=20$) con 700 épocas y \textbf{pSGLD}
(\texttt{use\_sgld=True}). Se usa pSGLD (no Adam) para que el nivel de
ruido sea coherente con $\varepsilon$: con $\varepsilon=0$ el ruido es
exactamente cero. Las 20 semillas son necesarias para estimar la media
y la varianza del gap con fiabilidad estadística.
...
conjunto de test de $N_\text{test} = 10\,000$ puntos i.i.d.\ de make\_moons (semilla fija $= 999$).
...
\textbf{Figura (G\_convergence\_problem.png).} Layout 1×3:
\begin{itemize}
  \item \textbf{G1 — Convergencia de la pérdida con $N$:} BCE\_test
        (línea continua) y BCE\_train (línea discontinua), media
        $\pm 1\sigma$ sobre 20 seeds, vs.\ $N$ en escala log en $x$, para
        $\varepsilon=0$ y $\varepsilon=0.01$. Evidencia visual de que
        BCE\_test$(N) \to J^*_\infty$ al crecer $N$. La regularización
        entrópica ($\varepsilon=0.01$) queda por debajo de $\varepsilon=0$
        para $N$ pequeño: actúa como prior que compensa la falta de datos.
  \item \textbf{G2 — Gap de generalización:}
        $\text{gap}(N) = \text{BCE\_test} - \text{BCE\_train}$ vs.\ $N$.
        Para $N$ pequeño el modelo sobreajusta (BCE\_train baja pero
        BCE\_test alta), de ahí el gap grande. Al crecer $N$ el gap se
        reduce porque la distribución empírica aproxima mejor a $\gamma_0$
        y hay menos sobreajuste.
  \item \textbf{G3 — Tasa de convergencia del gap (log-log):}
        $\log\,\text{gap}(N)$ vs.\ $\log N$ con ajuste lineal
        (mínimos cuadrados sobre $N \geq 100$), cuya pendiente es $-\alpha$.
        Se excluyen $N < 100$ porque en el régimen escaso el gap no sigue
        aún la ley de potencias asintótica. Los resultados empíricos son:
        $\varepsilon=0$: $\alpha \approx 0.78$;
        $\varepsilon=0.01$: $\alpha \approx 0.85$.
        Ambas tasas son más rápidas que la referencia $N^{-0.5}$.

        \textit{Cálculo de $\alpha$.} Se asume que el gap sigue una ley de
        potencias $\text{gap}(N) \propto N^{-\alpha}$. Tomando logaritmos,
        $\log\,\text{gap} = -\alpha\log N + c$, que es una recta en escala
        log-log. El exponente $\alpha$ se estima como la pendiente (con signo
        cambiado) de la regresión lineal por mínimos cuadrados ordinarios sobre
        los pares $(\log N,\,\log\,\text{gap})$ con $N \geq 100$.

        \textit{Referencia clásica $N^{-1/2}$.} La tasa $\alpha = 0.5$
        corresponde a la velocidad de convergencia de la medida empírica
        $\hat{\gamma}_N$ hacia la medida continua $\gamma_0$ garantizada por el
        \textbf{teorema de Glivenko-Cantelli}: bajo hipótesis
        estándar de momentos, la distancia de Wasserstein (o la norma supremo
        del proceso empírico) entre $\hat{\gamma}_N$ y $\gamma_0$ es
        $O(N^{-1/2})$. Esta cota se usa aquí como referencia natural para
        juzgar si la regularización entrópica acelera o no la convergencia.
\end{itemize}

% ============================================================
\subsection{Experimento H: Diagnóstico de pSGLD}
\label{sec:exp_h}

\textbf{Motivación.} El Experimento~G muestra que pSGLD converge en pérdida,
pero no garantiza que la distribución empírica de parámetros $\hat{\nu}^*$
se acerque al prior de Gibbs $\nu^\infty \propto \exp(-\ell(a)/\varepsilon)$.
De hecho, $\mathrm{KL}(\nu_N \| \nu^\infty) = +\infty$ cuando $\nu_N$ es
una suma de $M=64$ deltas de Dirac (medida discreta vs.\ continua).
El Experimento~H cuantifica esta brecha con métricas que no requieren
absolutamente continuidad: MMD$^2$ y distancia de Wasserstein $W_1$.

\textbf{Protocolo.}
\begin{enumerate}
  \item Se entrena un modelo con pSGLD durante 800 épocas y se recogen
        2\,000 instantáneas de la trayectoria (submuestreo cada 10 pasos),
        obteniendo $\hat{\nu}^*$ como promedio ergódico de la cadena.
  \item Se generan 8\,000 muestras de $\nu^\infty$ mediante
        \textbf{ULA} (Unadjusted Langevin Algorithm) con step adaptativo
        $\eta = \min(5\times10^{-3},\, 0.5\,\varepsilon)$.
        El step adaptativo es necesario porque la constante de Lipschitz
        de $\nabla U(a) = \nabla\ell(a)/\varepsilon$ escala como $1/\varepsilon$.
  \item Se calculan: MMD$^2(\nu_N, \hat{\nu}^*)$,
        MMD$^2(\hat{\nu}^*, \nu^\infty)$ y $W_1$ por dimensión entre
        $\hat{\nu}^*$ y $\nu^\infty$.
\end{enumerate}

\textbf{MMD$^2$ con kernel RBF.}
\[
  \mathrm{MMD}^2(P, Q) = \E_{x,x'\sim P}[k(x,x')]
  - 2\,\E_{x\sim P,y\sim Q}[k(x,y)]
  + \E_{y,y'\sim Q}[k(y,y')],
\]
donde $k(x,y) = \exp(-\|x-y\|^2 / (2\sigma^2))$ y $\sigma$ se fija
por la heurística de la mediana: $\sigma^2 = \mathrm{mediana}(\|x_i - x_j\|^2)/2$.

\textbf{Resultados.} MMD$^2(\nu_N, \hat{\nu}^*) \approx 0$
(la trayectoria de pSGLD se mezcla bien internamente), pero
MMD$^2(\hat{\nu}^*, \nu^\infty) = 0.217$
(la distribución estacionaria empírica está lejos del prior de Gibbs).
Esto cuantifica con precisión el problema teórico: pSGLD es internamente
consistente, pero no implementa fielmente la regularización entrópica
del paper de Daudin-Delarue.

\textbf{Figura (H\_psgld\_diagnostic.png).} Layout 2×2:
\begin{itemize}
  \item H1 — Histogramas 1D de $\hat{\nu}^*$ vs.\ $\nu^\infty$ por dimensión.
  \item H2 — Barras de MMD$^2$: $\nu_N$ vs.\ $\hat{\nu}^*$, y $\hat{\nu}^*$ vs.\ $\nu^\infty$.
  \item H3 — $W_1$ por dimensión entre $\hat{\nu}^*$ y $\nu^\infty$.
  \item H4 — Dispersión 2D con KDE de ambas distribuciones proyectadas.
\end{itemize}

% ============================================================
\subsection{Experimento I: MMD y Sinkhorn como regularizadores (clasificación)}
\label{sec:exp_i}

\textbf{Motivación.} Dado que $\mathrm{KL}(\nu_N\|\nu^\infty)=+\infty$,
se proponen MMD$^2$ y la divergencia de Sinkhorn como alternativas bien definidas
para medir y penalizar la distancia entre $\nu_N$ y $\nu^\infty$.
A diferencia del KL, estas métricas no requieren absolutamente continuidad.

\textbf{Objetivo de entrenamiento con MMD.}
\[
  J_\text{MMD} = \mathcal{L}(X, y) + \varepsilon\cdot\mathrm{MMD}^2(\nu_N,\,\nu^\infty),
\]
donde $\mathcal{L}$ es la BCE (clasificación) o MSE (regresión).
Para el término de entrenamiento se usa el estimador \emph{biased}
(denominador $n^2$, no $n(n-1)$), más estable para $M=64$ pequeño.
El término cruzado $\E[k(x,y)]$ con $\nu^\infty$ se precomputa una sola
vez antes del bucle de entrenamiento.

\textbf{Objetivo de entrenamiento con Sinkhorn (debiased).}
La divergencia de Sinkhorn regularizada con blur $\sigma_\text{blur}$ es:
\[
  S_\sigma(P, Q) = \mathrm{OT}_\sigma(P,Q)
  - \tfrac{1}{2}\mathrm{OT}_\sigma(P,P)
  - \tfrac{1}{2}\mathrm{OT}_\sigma(Q,Q),
\]
donde $\mathrm{OT}_\sigma$ es el transporte óptimo con costo
$C_{ij} = \|x_i - y_j\|^2 / \max(C)$ (normalizado a $[0,1]$)
y regularización entrópica $\sigma_\text{blur}$.
Se implementa en dominio logarítmico para estabilidad numérica.
Los términos de auto-transporte $\mathrm{OT}_\sigma(Q,Q)$ con $\nu^\infty$
se precomputan una sola vez.

\textbf{Protocolo.} Se comparan tres métodos en make\_moons con $N=400$,
$\varepsilon=0.01$, 700 épocas y 10 semillas:
pSGLD (\texttt{use\_sgld=True}), MMD-reg (\texttt{use\_mmd=True})
y Sinkhorn-reg (\texttt{use\_sinkhorn=True}, \texttt{blur}$=0.05$).

\textbf{Resultados.} Los tres métodos convergen a accuracy similares en
make\_moons ($\sim 90\%$), con MMD$^2(\nu_\text{reg}, \hat{\nu}^*_\text{pSGLD})
\approx 0.014$--$0.019$. En clasificación la tarea es suficientemente
sencilla para que el regularizador no discrimine entre métodos.

% ============================================================
\subsection{Experimento J: Regresión en California Housing}
\label{sec:exp_j}

\textbf{Motivación.} En clasificación (Exp.~I), el regularizador representa
$\varepsilon\cdot\mathrm{reg}/\text{BCE} \approx 333\%$: domina la pérdida.
En regresión con $\varepsilon=0.01$, la fracción es solo $\approx 0.25\%$:
el regularizador es prácticamente invisible. La comparación en regresión
somete los métodos a un régimen muy distinto.

\textbf{Dataset.} California Housing (8 variables, 20\,640 viviendas).
Se proyecta a $\R^2$ mediante PCA (varianza explicada: 48.9\%),
se normaliza con \texttt{StandardScaler} y se sub-muestrea:
$N_\text{train}=800$, $N_\text{test}=400$.
La variable objetivo (precio mediano) se estandariza a media cero y varianza unitaria.

\textbf{Métrica.} $R^2 = 1 - \mathrm{SS_{res}}/\mathrm{SS_{tot}}$
(coeficiente de determinación), además del MSE.
El modelo usa \texttt{task='regression'}: la cabeza lineal predice un escalar,
la pérdida es MSE (en lugar de BCE).

\textbf{Resultados} (500 épocas, $\varepsilon=0.01$):
\begin{center}
\begin{tabular}{lccc}
\toprule
Método & MSE$_\text{test}$ & $R^2_\text{test}$ \\
\midrule
pSGLD    & 0.845 & 0.158 \\
MMD-reg  & 0.782 & 0.221 \\
Sinkhorn-reg & 0.770 & 0.232 \\
\bottomrule
\end{tabular}
\end{center}
El techo de $R^2 \approx 0.25$ refleja la pérdida de información del PCA(2)
(48.9\% de varianza explicada), no necesariamente los límites del método.
La hipótesis de que la inferioridad de pSGLD puede ser un artefacto de
escala de $\varepsilon$ se investiga en el Experimento~K.

\textbf{Figura (J\_california\_regression.png).} Layout 2×3:
curvas de convergencia MSE/R², valores del regularizador,
predicho vs.\ real, comparación $W_1$, dispersión 2D en espacio PCA.

% ============================================================
\subsection{Experimento K: Barrido de $\varepsilon$ en regresión}
\label{sec:exp_k}

\textbf{Motivación.} El valor $\varepsilon=0.01$ genera una fracción de
regularización muy diferente entre clasificación y regresión:
\[
  \text{Clasificación (Exp.~I):}\quad
  \frac{\varepsilon\cdot\mathrm{reg}}{\text{BCE}} \approx 3.33
  \qquad\text{vs.}\qquad
  \text{Regresión (Exp.~J):}\quad
  \frac{\varepsilon\cdot\mathrm{reg}}{\text{MSE}} \approx 0.003.
\]
La inferioridad de pSGLD en Exp.~J puede ser un artefacto del valor de
$\varepsilon$, no una propiedad fundamental del método.

\textbf{Protocolo.} Para cada $\varepsilon \in \{0.001, 0.01, 0.1, 0.5, 1, 5\}$:
\begin{enumerate}
  \item Se regeneran las muestras del prior $\nu^\infty \propto \exp(-\ell(a)/\varepsilon)$
        usando ULA con step $\eta = \min(5\times10^{-3},\, 0.5\,\varepsilon)$.
        Es obligatorio regenerar porque $\nu^\infty$ depende de $\varepsilon$.
  \item Se entrenan pSGLD, MMD-reg y Sinkhorn-reg durante 500 épocas.
  \item Se registran MSE, $R^2$ (train y test) y la fracción de regularización.
\end{enumerate}
...
\begin{tabular}{lcccc}
\toprule
$\varepsilon$ & pSGLD & MMD & Sinkhorn & $\varepsilon$ óptimo de\\
\midrule
0.001 & \textbf{0.236} & 0.205 & 0.218 & pSGLD\\
0.01  & 0.158 & \textbf{0.221} & 0.232 & MMD\\
0.1   & $-0.000$ & 0.216 & \textbf{0.253} & Sinkhorn\\
...
\textbf{Conclusiones:}
\begin{itemize}
  \item \textbf{pSGLD} solo es competitivo para $\varepsilon \leq 0.001$.
        Para $\varepsilon \geq 0.1$ el ruido Langevin $\sqrt{2\eta\varepsilon M}\,\xi$
        domina el gradiente y el modelo diverge.
        Su ``mejor'' R$^2=0.236$ se alcanza cuando $\varepsilon \to 0$,
        es decir, cuando la regularización entrópica desaparece.
  \item \textbf{MMD} es notablemente robusto: $R^2 \approx 0.21$ para todo
        $\varepsilon \in [0.001, 1.0]$. La fracción de regularización se mantiene
        siempre pequeña porque el estimador MMD$^2$ escala diferente a $1/\varepsilon$.
  \item \textbf{Sinkhorn} tiene un óptimo claro en $\varepsilon=0.1$
        ($R^2=0.253$, el mejor global), pero se degrada para $\varepsilon \geq 1$
        porque la fracción de regularización supera el 200\%.
  \item El ranking definitivo al $\varepsilon$ óptimo de cada método es
        Sinkhorn $(0.253)$ $>$ pSGLD $(0.236)$ $>$ MMD $(0.221)$.
        La inferioridad de pSGLD en Exp.~J no era solo un artefacto de
        escala: Sinkhorn supera genuinamente a los demás cuando se calibra bien.
\end{itemize}

\textbf{Figura (K\_epsilon\_sweep.png).} Layout 2×2:
K1 ($R^2_\text{test}$ vs.\ $\varepsilon$ en log),
K2 (train vs.\ test $R^2$ para detectar overfitting),
K3 (fracción de regularización $\varepsilon\cdot\text{reg}/\text{MSE}$ en log-log),
K4 (barras comparativas al $\varepsilon$ original, compartido óptimo e individual óptimo).
...
Bloque & Descripción \\
\midrule
Datasets & make\_moons (A–G, I), make\_circles (F), California Housing PCA(2) (J, K) \\
...
Clasificador/Regresor & $\ell = W \cdot X_T + b$,\; $W \in \R^{1\times 2}$,\; $b \in \R$ (3 total) \\
Total parámetros & 387 \\
Pérdida (clasificación) & $J = \text{BCE} + \varepsilon \cdot \mathcal{E}$,\; $\varepsilon = 0.01$ \\
Pérdida (regresión) & $J = \text{MSE} + \varepsilon \cdot \mathcal{E}$,\; $\varepsilon \in [0.001, 5]$ \\
Optimizador (A, D) & Adam \cite{kingma2015adam},\; $\alpha_0=0.01$,\; cosine annealing \\
Optimizador (C) & SGD,\; $\alpha=0.01$ constante,\; sin ruido \\
Optimizador pSGLD (B,E–H,I,J,K) & \cite{li2016psgld},\; $\theta \leftarrow \text{Adam}(\theta,\nabla J) + \sqrt{2\eta\varepsilon M}\,\xi$,\; $M = \min(1/\sqrt{\hat{v}}+\delta,\,1)$ \\
Regularizador MMD (I,J,K) & $\mathcal{E} = \mathrm{MMD}^2(\nu_N, \nu^\infty)$,\; kernel RBF,\; $\sigma$ por heurística mediana \cite{gretton2012kernel} \\
Regularizador Sinkhorn (I,J,K) & $\mathcal{E} = S_\sigma(\nu_N, \nu^\infty)$,\; Sinkhorn debiased,\; log-domain \cite{feydy2019interpolating} \\
Prior $\nu^\infty$ (H–K) & ULA,\; $\eta = \min(5\times10^{-3},\, 0.5\varepsilon)$,\; $n_\text{burnin}=5000$ \\
...
\bibitem{li2016psgld}
  ...
  \textit{Stochastic Gradient Langevin Dynamics Algorithms with Adaptive Drifts},
...
\bibitem{gretton2012kernel}
  ...
  \textit{A Kernel Two-Sample Test},
...
\bibitem{feydy2019interpolating}
  ...
  in \textit{International Conference on Artificial Intelligence and Statistics
  (AISTATS)}, 2019.
