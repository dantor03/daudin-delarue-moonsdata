...
\begin{itemize}
  \item \textbf{B1 — Curvas de convergencia:} $J$ total, accuracy y
        ...
        Los casos extremos merecen atención especial: con $\varepsilon = 0.5$
        (curva morada), la penalización entrópica es tan fuerte que la accuracy
        no llega a $1.0$ — la regularización impide al modelo ajustarse
        completamente a los datos. Con $\varepsilon = 0.1$ (curva azul), la
        pérdida $J$ no llega a cero: el término entrópico mantiene un nivel
        asintótico elevado, aunque la clasificación sea correcta. En ambos
        casos, cuanto mayor es $\varepsilon$, mayor es el nivel estacionario
        de $J^*$ y más ruidosas son las curvas (el ruido pSGLD escala como
        $\sqrt{\varepsilon}$).
...
\textbf{Observación sobre $\varepsilon$ grande.}
Con $\varepsilon = 0.5$ el término de regularización domina la pérdida $J$,
lo que eleva el nivel asintótico de las curvas y produce oscilaciones
mayores por el ruido pSGLD (que escala como $\sqrt{\varepsilon}$). Esto no
compromete la clasificación — la accuracy sigue siendo alta — pero muestra
que valores de $\varepsilon$ demasiado grandes hacen que la exploración de
$\nu^*$ sea ruidosa y el nivel estacionario de $J$ se aleje de la BCE pura.
