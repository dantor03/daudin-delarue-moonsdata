...
pSGLD + cosine annealing & \texttt{use\_sgld=True} & B, E, F & Muestreo de $\nu_t^*$ \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
\multicolumn{4}{l}{\textit{Solo pSGLD (experimentos B, E, F):}} \\
$\sqrt{2\eta_s\varepsilon M_s}$ & variable & B, E, F & Escala del ruido precondicionado (por componente) \\
$\lambda$ & $1$ & B, E, F & Cota superior de $M_s$ (evita explosión en dirs.\ planas) \\
\bottomrule
\end{tabular}
...
Optimizador (B, E, F) & pSGLD \cite{li2016psgld},\; $\theta \leftarrow \text{Adam}(\theta,\nabla J) + \sqrt{2\eta\varepsilon M}\,\xi$,\; $M = \min(1/\sqrt{\hat{v}}+\delta, 1)$ \\
