...
Se generan $N = 400$ pares $(x_i, y_i)$ con la función \texttt{make\_moons} de
\texttt{scikit-learn}. Cada punto pertenece a una de dos clases entrelazadas:
$y_i \in \{0, 1\}$, con 200 puntos por clase.
...
$\gamma_0 = \frac{1}{N}\sum_{i=1}^N \delta_{(x_0^{(i)}, y_i)}$ a la medida
empírica de los datos de entrenamiento.
...
\end{equation}
En la implementación, $\mathcal{E}$ se calcula recorriendo todos los
parámetros escalares $\theta_j$ de \texttt{MeanFieldVelocity} de forma
unificada.
...
SGLD + cosine annealing & \texttt{use\_sgld=True} & B, E, F & Muestreo de $\nu_t^*$ \\
\bottomrule
\end{tabular}
\end{center}
...
Para los Experimentos B, E y F, se usa SGLD \cite{welling2011sgld}: la
discretización de Euler-Maruyama de la SDE
\begin{equation}
  d\theta_t = -\nabla J(\theta_t)\,dt + \sqrt{2\varepsilon}\,dW_t,
\end{equation}
donde $W_t$ es un movimiento Browniano estándar. El esquema discreto con paso
$\eta_s$ (tasa de aprendizaje en el paso $s$) es:
\begin{equation}
  \label{eq:sgld}
  \theta^s = \theta^{s-1} - \eta_s\,\nabla J(\theta^{s-1})
             + \sqrt{2\,\eta_s\,\varepsilon}\;\xi^s,
  \qquad \xi^s \sim \mathcal{N}(0, I).
\end{equation}

El ruido $\sqrt{2\eta_s\varepsilon}\,\xi^s$ implementa el \textbf{término de
entropía} $-H(\nu_t)$ de la KL: hace que los parámetros exploren la
distribución de Gibbs $\nu^* \propto \exp(-J(\theta)/\varepsilon)$ en lugar
de colapsar a un estimador puntual (MAP). El término de energía
$\mathcal{E}(\theta)$ en~\eqref{eq:energia} actúa como el prior $\nu^\infty$.

Con $\varepsilon = 0$ el ruido es exactamente cero, recuperando SGD estándar.
La escala $\sqrt{2\eta_s\varepsilon}$ está derivada de la teoría del paper
(sección~1.3): $\varepsilon$ es la temperatura entrópica y $\eta_s$ el paso
de Euler-Maruyama. El ruido se inyecta \emph{después} del paso de gradiente
y \emph{antes} del scheduler (para usar el lr actual $\eta_s$, no el
siguiente).

La tasa de aprendizaje sigue cosine annealing ($\eta_s \to 0$), lo que
implementa \emph{simulated annealing} natural y satisface las condiciones de
Robbins-Monro: $\sum_s \eta_s = \infty$, $\sum_s \eta_s^2 < \infty$.
...
$n_\text{epochs}$ & 800 (A), 700 (B–F) & todos & Épocas de entrenamiento \\
...
\midrule
\multicolumn{4}{l}{\textit{Solo SGLD (experimentos B, E, F):}} \\
$\sqrt{2\eta_s\varepsilon}$ & variable & B, E, F & Escala del ruido de Langevin \\
\bottomrule
\end{tabular}
...
$\varepsilon \in \{0, 10^{-3}, 10^{-2}, 0.1, 0.5\}$ con \textbf{SGLD}
(\texttt{use\_sgld=True}), manteniendo la misma semilla de inicialización.
El uso de SGLD es coherente con el rol de $\varepsilon$ como temperatura
entrópica: el ruido Langevin $\sqrt{2\eta_s\varepsilon}\,\xi$ hace que los
parámetros exploren $\nu^* \propto \exp(-J(\theta)/\varepsilon)$. Con
$\varepsilon = 0$ el ruido es exactamente cero (SGD puro).

\textbf{Magnitudes analizadas.}

\begin{itemize}
  \item \textbf{B1 — Curvas de convergencia:} $J$ total, accuracy y
        $\mathcal{E}$ vs.\ época para cada $\varepsilon$.
...
\begin{itemize}
  ...
  \item \textbf{D2 — Inicialización fija, $\gamma_0$ variable:}
        \texttt{init\_seed}$=4$, \texttt{data\_seed}$\in\{0,\ldots,9\}$.
...
solo para runs con acc $\geq 0.90$, sobre una nube de puntos de todos los
datasets.
...
$\varepsilon = 0.01$, 500 épocas y \textbf{SGLD} (\texttt{use\_sgld=True},
para explorar la distribución de Gibbs $\nu^*$ en lugar del MAP):
\begin{itemize}
  \item \textbf{E-1:} \texttt{data\_seed}$=42$ fijo, \texttt{init\_seed}
        $\in \{0,\ldots,9\}$ variable.
  \item \textbf{E-2:} \texttt{init\_seed}$=4$ fijo, \texttt{data\_seed}
        $\in \{0,\ldots,9\}$ variable.
\end{itemize}
...
\textbf{Protocolo.} Dos sub-experimentos con $n_\text{seeds} = 10$,
$n_\text{epochs} = 700$ y $\varepsilon = 0.01$:
\begin{itemize}
...
Bloque & Descripción \\
\midrule
Datasets & make\_moons (A–E) y make\_circles (F), $N=400$, normalizados \\
...
Optimizador (B, E, F) & SGLD \cite{welling2011sgld},\; $\theta \leftarrow \theta - \eta\nabla J + \sqrt{2\eta\varepsilon}\,\xi$ \\
